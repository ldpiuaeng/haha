...
public class IP {
	...
	private static String FILEPATH = "ip/17monipdb.dat";
	...
	private static void load() {
		...
		try {
			dataBuffer = ByteBuffer
					.allocate(Long.valueOf(ipFile.length()).intValue());
	...
	public static String[] findProCity(String ip) {
		String[] proCity = new String[]{"未知", "未知"};
		String[] tmp = find(ip);
		if (tmp != null) {
			proCity = tmp.length > 2
					? new String[]{tmp[1], tmp[2]}
					: (tmp.length > 1
							? new String[]{tmp[1], "未知"}
							: new String[]{"未知", "未知"});
		}
	...
	public static void main(String[] args) {
		...
		IP.load(Thread.currentThread().getContextClassLoader()
				.getResource(FILEPATH).getFile());

		System.out.println(Arrays.toString(IP.findProCity("219.238.5.242")));
	}
